...
def run_a_star(start,goal):
    ...
    #print('Starting Greedy Best First search from grid cell ', start_node.state, ' to goal cell ', goal)
    print('Starting Uniform Cost search from grid cell ', start_node.state, ' to goal cell ', goal)
...
def get_neighbors(g, closed):
    ...
    x = g[0]
    y = g[1]
    possible_neighbors = [(x-1,y-1), (x-1,y), (x-1,y+1), (x+1,y-1), (x+1,y), (x+1,y+1), (x,y-1), (x,y+1)]
    #print('Possible Neighbors: ', possible_neighbors)
    not_closed = []
    for each in possible_neighbors:
        if each in closed:
            a12 = 1
            #print(each," is in Closed")
        elif each in dmap.occupied:
            a12 = 1
            #print(each," is Occupied")
        else:
            not_closed.append(each)

    #print('Not Closed: ', not_closed)
    #on_map = []
    neighbors = not_closed
    #print('Possible ', possible_neighbors)

...
def filter_by_distance(cloud, distance=10):
    cloud_distance=((cloud[:,0]**2)+(cloud[:,1]**2))**0.5
    cloud_distance(cloud_distance<distance)
    return cloud[cloud_distance]
...
def euclidean_clustering(cloud, threshold=0.5):
    queue = Queue()
    cluster_labels = [0] * len(cloud)
    for i in range(len(cloud)):
        queue = Queue()
        if cluster_labels[i]==0:
            queue.put(cloud[i])
            cluster_labels[i]=i+1
            while not queue.empty():
                q=queue.get()
                dist = ((q[0]-cloud[:,0])**2 + (q[1]-cloud[:,1])**2 + (q[2]-cloud[:,2])**2)**0.5
                n=(0<dist)&(dist<threshold)
                for j in range(len(n)):
                    if n[j] == 0 and cluster_labels[j]==0:
                        queue.put(cloud[j])
...
def filter_clusters(cluster_labels, min_num_points=100):
    for i in range(max(cluster_labels)):
        if (cluster_labels==i+1).sum() < min_num_points:
            cluster_labels[cluster_labels==i+1] = 0
            cluster_labels[cluster_labels>i+1] = cluster_labels[cluster_labels>i+1] - 1
    return cluster_labels
...
def get_bounding_boxes(cloud, cluster_labels):
    ...
    for i in range(max(cluster_labels)):
        ...
        y = cld[:,1]
        z = cld[:,3]
        boxes.append(np.array([[min(x),min(y),min(z)],[min(x),min(y),max(z)],
        [min(x),max(y),min(z)],[min(x),max(y),max(z)],[max(x),min(y),min(z)],

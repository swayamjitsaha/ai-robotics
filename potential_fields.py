...
def goal_force( ):
    ...
    angle = math.atan2(goal[1] - robot[1], goal[0] - robot[0]) - robot[2]
    x_var = math.cos(angle) * strength
    y_var = math.sin(angle) * strength
    force_to_goal = [x_var,y_var]
    #########################
    # PART A : END
    #########################
...
def obstacle_force():  
    ...
    for i in range(len(laser_scan.ranges)):
        ...
        x_force = math.cos(cur_angle) * strength
        y_force = math.sin(cur_angle) * strength
        force_from_obstacles[0] -= x_force
        force_from_obstacles[1] -= y_force
        #########################
        # PART B : END
        #########################
...
def get_pf_magnitude_linear(distance):
    ...
    if distance_threshold > distance:
        return max_strength*distance/distance_threshold
    #########################
    #  PART C : END
    #########################
